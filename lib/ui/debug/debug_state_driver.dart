...
/// Control column: cycles the fake state through every scenario so each
...
class DebugStateDriver extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Watching the state (not just the notifier) rebuilds the label.
    ref.watch(controlViewStateProvider);
    ...
    return Container(
      ...
      child: Row(
        children: [
          chip(ControlKeys.debugPrev, '‹', () => notifier.cycleScenario(step: -1)),
          Expanded(
            child: Text(
              'DEBUG · ${notifier.scenario.label} · ${variant.name}',
          ...
          chip(ControlKeys.debugNext, '›', () => notifier.cycleScenario()),
